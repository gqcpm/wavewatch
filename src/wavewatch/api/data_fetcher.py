...
class StormglassDataFetcher:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not api_key:
            raise ValueError(
                "Stormglass API key is required. Set STORMGLASS_API_KEY environment variable or pass api_key parameter."
            )
            ...
            if response.status_code == 200:
                ...
                if "stations" in data and len(data["stations"]) > 0:
                    ...
                    metadata = {
                        ...
                        "tidepredoffsets_url": (
                            station_info.get("tidepredoffsets", {}).get("self")
                            if isinstance(station_info.get("tidepredoffsets"), dict)
                            else None
                        ),
                    }
                    ...
                    if metadata["type"] == "S" and metadata["tidepredoffsets_url"]:
                        try:
                            offsets_response = requests.get(
                                metadata["tidepredoffsets_url"], timeout=10
                            )
                            if offsets_response.status_code == 200:
                                offsets_data = offsets_response.json()
                                metadata["tidepredoffsets"] = {
                                    "time_high": offsets_data.get(
                                        "timeOffsetHighTide", 0
                                    ),
                                    "time_low": offsets_data.get(
                                        "timeOffsetLowTide", 0
                                    ),
                                    "height_high": offsets_data.get(
                                        "heightOffsetHighTide", 1.0
                                    ),
                                    "height_low": offsets_data.get(
                                        "heightOffsetLowTide", 1.0
                                    ),
                                }
                        except Exception as e:
                            print(
                                f"Warning: Could not fetch tide offsets for {station_id}: {e}"
                            )
    ...
    def _get_noaa_tide_data(self, beach_name: str, target_date: str = None) -> Dict:
        ...
        try:
            ...
            if (
                station_metadata
                and station_metadata.get("type") == "S"
                and station_metadata.get("reference_id")
            ):
                ...
                try:
                    ...
                    if predictions_response.status_code == 200:
                        ...
                        if not predictions_list:
                            return {
                                "error": f"No tide predictions available from reference station {reference_station_id}"
                            }
                        ...
                        return {
                            "error": f"Failed to fetch predictions from reference station {reference_station_id}: HTTP {predictions_response.status_code}"
                        }
                except Exception as e:
                    return {
                        "error": f"Error fetching predictions from reference station: {str(e)}"
                    }
            else:
                ...
                try:
                    ...
                    if predictions_response.status_code == 200:
                        ...
                        if not predictions_list:
                            return {
                                "error": f"No tide predictions available for station {station_id}"
                            }
                        ...
                        return {
                            "error": f"Failed to fetch predictions for station {station_id}: HTTP {predictions_response.status_code}"
                        }
                except Exception as e:
            ...
            return {
                "error": f"Error fetching NOAA tide data: {str(e)}\n{traceback.format_exc()}"
            }
                ...
                hourly_conditions.append(
                    {
                        ...
                        "wave_direction": hour_data.get("waveDirection", {}).get(
                            "noaa", "N/A"
                        ),
                        "wave_period": hour_data.get("wavePeriod", {}).get(
                            "noaa", "N/A"
                        ),
                        "wind_speed": wind_speed_mph,
                        "wind_direction": hour_data.get("windDirection", {}).get(
                            "noaa", "N/A"
                        ),
                        ...
                        "cloud_cover": hour_data.get("cloudCover", {}).get(
                            "noaa", "N/A"
                        ),
                        "precipitation": hour_data.get("precipitation", {}).get(
                            "noaa", "N/A"
                        ),
                    }
                )
    ...
    def get_best_surf_times(
        self,
        beach_name: str,
        lat: Optional[float] = None,
        lng: Optional[float] = None,
        min_wave_height: float = 2.0,
        target_date: str = None,
    ) -> Dict:
        """
        ...
        try:
            ...
            for hour in hourly_conditions:
                try:
                    wave_height = (
                        float(hour["wave_height"])
                        if hour["wave_height"] != "N/A"
                        else 0
                    )
                    wind_speed = (
                        float(hour["wind_speed"]) if hour["wind_speed"] != "N/A" else 0
                    )
                    ...
                    if wave_height >= min_wave_height:
                        ...
                        wind_score = max(
                            0, 20 - wind_speed
                        )  # Max score of 20 for no wind
                        wave_score = min(
                            30, wave_height * 10
                        )  # Max score of 30 for 3+ ft waves
        ...
        try:
            ...
            try:
                ...
                current_hour = data["hours"][
                    0
                ]  # Default to first hour (00:00) if index is out of range
            ...
            return {
                ...
                "current_conditions": {
                    "wave_height": wave_height_ft,
                    "wave_direction": current_hour.get("waveDirection", {}).get(
                        "noaa", "N/A"
                    ),
                    "wave_period": current_hour.get("wavePeriod", {}).get(
                        "noaa", "N/A"
                    ),
                    "wind_speed": wind_speed_mph,
                    "wind_direction": current_hour.get("windDirection", {}).get(
                        "noaa", "N/A"
                    ),
                    "water_temperature": water_temp_f,
                    "air_temperature": air_temp_f,
                    ...
                    "visibility": visibility_mi,
                    "cloud_cover": current_hour.get("cloudCover", {}).get(
                        "noaa", "N/A"
                    ),
                    "precipitation": current_hour.get("precipitation", {}).get(
                        "noaa", "N/A"
                    ),
                },
            }
