                    ...
                    df_data.append(
                        {
                            "time": hour_time,
                            "wave_height": (
                                float(hour.get("wave_height", 0))
                                if hour.get("wave_height") != "N/A"
                                and hour.get("wave_height") is not None
                                else 0
                            ),
                            "wave_period": (
                                float(hour.get("wave_period", 0))
                                if hour.get("wave_period") != "N/A"
                                and hour.get("wave_period") is not None
                                else 0
                            ),
                            "wind_speed": (
                                float(hour.get("wind_speed", 0))
                                if hour.get("wind_speed") != "N/A"
                                and hour.get("wind_speed") is not None
                                else 0
                            ),
                            "tide": (
                                float(hour.get("tide", 0))
                                if hour.get("tide") != "N/A"
                                and hour.get("tide") is not None
                                else 0
                            ),
                            "air_temperature": (
                                float(hour.get("air_temperature", 0))
                                if hour.get("air_temperature") != "N/A"
                                and hour.get("air_temperature") is not None
                                else 0
                            ),
                            "humidity": (
                                float(hour.get("humidity", 0))
                                if hour.get("humidity") != "N/A"
                                and hour.get("humidity") is not None
                                else 0
                            ),
                            "water_temperature": (
                                float(hour.get("water_temperature", 0))
                                if hour.get("water_temperature") != "N/A"
                                and hour.get("water_temperature") is not None
                                else 0
                            ),
                        }
                    )
...
def create_surf_map(beach_name, coordinates, current_conditions):
    ...
    try:
        ...
        wave_dir_rad = (
            math.radians(float(wave_direction)) if wave_direction != "N/A" else 0
        )
        wind_dir_rad = (
            math.radians(float(wind_direction)) if wind_direction != "N/A" else 0
        )
...
def get_one_sentence_summary(
    beach_name, current_conditions, summarizer, selected_date=None
):
    """Generate a one-sentence summary of current surf conditions using AI."""
    try:
        # Use the new summarizer method
        return summarizer.get_one_sentence_summary(
...
# Configure page
st.set_page_config(page_title="WaveWatch", page_icon="🏄‍♂️", layout="wide")
...
def main():
    ...
    if not stormglass_api_key:
        st.error(
            "STORMGLASS_API_KEY not found in environment variables. Please check your .env file."
        )
        st.stop()
    ...
    with col1:
        ...
        show_real_data = st.checkbox(
            "Show Real Surf Data",
            value=True,
            help="Display actual surf conditions from Stormglass API",
        )
        show_ai_analysis = st.checkbox(
            "Show AI Analysis", value=True, help="Display AI-generated surf analysis"
        )
                    ...
                    try:
                        ...
                        if show_real_data:
                            ...
                            st.session_state.real_data = (
                                data_fetcher.get_current_conditions(
                                    surf_beach, target_date=date_str
                                )
                            )
                            st.session_state.hourly_data = (
                                data_fetcher.get_hourly_conditions(
                                    surf_beach, target_date=date_str
                                )
                            )
                            st.session_state.best_times = (
                                data_fetcher.get_best_surf_times(
                                    surf_beach, target_date=date_str
                                )
                            )
                        ...
                        if show_ai_analysis:
                            ...
                            combined_data = {
                                "current_conditions": st.session_state.real_data.get(
                                    "current_conditions", {}
                                ),
                                "hourly_conditions": st.session_state.hourly_data.get(
                                    "hourly_conditions", []
                                ),
                            }
                            st.session_state.ai_analysis = (
                                summarizer.get_surf_conditions(
                                    surf_beach, combined_data, selected_date
                                )
                            )
    ...
    with col2:
        ...
        if hasattr(st.session_state, "beach") and st.session_state.beach:
            date_display = (
                st.session_state.selected_date.strftime("%B %d, %Y")
            ...
            if st.session_state.show_real_data and hasattr(
                st.session_state, "real_data"
            ):
            ...
            if st.session_state.show_real_data and hasattr(
                ...
                if "error" in st.session_state.real_data:
                    ...
                    with col_b:
                        ...
                        if real_data["tide"] != "N/A":
                            ...
                            st.metric(
                                "Tide",
                                "N/A",
                                help="Tide data not available for this location",
                            )
                    ...
                    if (
                        "tide_data" in st.session_state.real_data
                        and "station_name" in st.session_state.real_data["tide_data"]
                    ):
                        st.info(
                            f"🌊 Tide data from: {st.session_state.real_data['tide_data']['station_name']} (Station ID: {st.session_state.real_data['tide_data']['station_id']})"
                        )
            ...
            if st.session_state.show_real_data and hasattr(
                ...
                if "error" not in st.session_state.hourly_data:
                    ...
                    for hour in hourly_data:
                        with st.expander(
                            f"🕐 {hour['time'][:16]} - {hour['wave_height']}ft waves"
                        ):
                            col_a, col_b = st.columns(2)
                            with col_a:
                                st.write(
                                    f"**Waves:** {hour['wave_height']}ft @ {hour['wave_period']}s"
                                )
                                st.write(
                                    f"**Wave Direction:** {hour['wave_direction']}°"
                                )
                                st.write(
                                    f"**Wind:** {hour['wind_speed']} mph @ {hour['wind_direction']}°"
                                )
                            with col_b:
            ...
            if st.session_state.show_real_data and hasattr(
                ...
                if "error" not in st.session_state.best_times:
                    ...
                    for i, time_slot in enumerate(best_times, 1):
                        st.write(
                            f"**{i}.** {time_slot['time'][:16]} - {time_slot['wave_height']}ft waves, {time_slot['wind_speed']} mph wind (Score: {time_slot['score']:.1f})"
                        )
            ...
            if st.session_state.show_real_data and hasattr(
                ...
                if "error" not in st.session_state.hourly_data:
                    ...
                    if charts:
                        ...
                        with col1:
                            st.plotly_chart(
                                charts["wave_height"], use_container_width=True
                            )
                            st.plotly_chart(
                                charts["wave_period"], use_container_width=True
                            )
                            st.plotly_chart(
                                charts["wind_speed"], use_container_width=True
                            )
                            st.plotly_chart(charts["tide"], use_container_width=True)

                        with col2:
                            st.plotly_chart(
                                charts["air_temperature"], use_container_width=True
                            )
                            st.plotly_chart(
                                charts["humidity"], use_container_width=True
                            )
                            st.plotly_chart(
                                charts["water_temperature"], use_container_width=True
                            )

            # Display AI analysis
            if st.session_state.show_ai_analysis and hasattr(
                st.session_state, "ai_analysis"
            ):
            ...
            st.info(
                "Enter a beach name and click 'Get Surf Conditions' to see the analysis."
            )
